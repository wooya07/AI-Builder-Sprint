...
export default function Home() {
  ...
  return <main>
    ...
    {view === "semester" && <section className="standalone-page"><div className="section-title"><div><p className="eyebrow">SEMESTER PLANNER</p><h1>학기 시간표 추천</h1></div><a href="/admin">강의 목록 가져오기 →</a></div><div className="semester-grid"><form className="panel timetable-form" onSubmit={generateTimetables}><fieldset className="timetable-checklist"><legend><span>시간표 체크리스트</span><small>나에게 맞는 수업 리듬을 선택하세요.</small></legend><div className="checklist-selects"><label>희망 학점<select value={targetCredits} onChange={(event) => setTargetCredits(event.target.value)}>{[6,9,12,15,18,21,24].map((credits) => <option key={credits} value={credits}>{credits}학점</option>)}</select></label><label>희망 첫 수업 시작 시간<select value={firstClassStart} onChange={(event) => setFirstClassStart(event.target.value)}><option value="">상관없음</option>{[9,10,11,12,13].map((hour) => <option key={hour} value={hour}>{hour}:00 이후</option>)}</select></label></div><div className="checklist-toggles"><label className="check"><span><b>점심시간 확보</b></span><input type="checkbox" checked={wantsLunch} onChange={(event) => setWantsLunch(event.target.checked)}/></label><label className="check"><span><b>저녁시간 확보</b></span><input type="checkbox" checked={wantsDinner} onChange={(event) => setWantsDinner(event.target.checked)}/></label><label>원하는 공강 요일<select value={freeDay} onChange={(event) => setFreeDay(event.target.value)}>{["월","화","수","목","금"].map((item) => <option key={item}>{item}</option>)}</select></label><label>최대 연강<select value={maxConsecutiveClasses} onChange={(event) => setMaxConsecutiveClasses(event.target.value)}><option value="">상관없음</option>{[1,2,3,4].map((count) => <option key={count} value={count}>{count}연강</option>)}</select></label><label>이동 시간<select value={travelMinutes} onChange={(event) => setTravelMinutes(event.target.value)}><option value="">상관없음</option>{[10,20,30,60].map((minutes) => <option key={minutes} value={minutes}>{minutes}분</option>)}</select></label><label>하루 최대 수업<select value={maxDailyClasses} onChange={(event) => setMaxDailyClasses(event.target.value)}><option value="">상관없음</option>{[2,3,4,5,6].map((count) => <option key={count} value={count}>{count}개</option>)}</select></label><label>희망 하교 시간<select value={endTime} onChange={(event) => setEndTime(event.target.value)}><option value="">상관없음</option>{[15,16,17,18,19,20].map((hour) => <option key={hour} value={hour}>{hour}:00 이전</option>)}</select></label></div></fieldset><button className="primary">나만의 시간표 만들기</button></form><div className="semester-results">{timetableMessage && <p className="error">{timetableMessage}</p>}{!timetables.length && !timetableMessage && <p className="empty-result">조건을 설정하면 추천 시간표가 여기에 표시됩니다.</p>}{timetables.map((table) => <article className="panel" key={`${table.title}-${table.score}`}><p className="eyebrow">{table.title}</p><h3>{table.total_credits}학점 · 점수 {table.score}</h3>{table.courses.map((course) => <div className="catalog-course" key={`${course.code}-${course.instructor}`}><b>{course.name}</b><span>{course.meetings.map((meeting) => `${meeting.day} ${meeting.start}:00–${meeting.end}:00`).join(" · ")}</span></div>)}<button className="apply-button" onClick={() => applyTimetable(table)}>이 시간표 적용</button></article>)}</div></div></section>}
